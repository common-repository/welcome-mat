...
maxAjax.prototype.hooks = function()
{
	...
	$(document).trigger('wm_maxajax_init'); // for hanging in other actions.
}
...
maxAjax.prototype.setFunction = function(type, new_function)
{
		...
		if (type == 'success')
			this.successHandler = new_function;
		if (type == 'error')
			 this.errorHandler = new_function;
}
...
window.maxFoundry.wm = {};
window.maxFoundry.wm.maxAjax = maxAjax;
//window.maxFoundry.maxajax.init();

jQuery(document).ready(function($) {
		ajaxObj = new window.maxFoundry.wm.maxAjax;
		ajaxObj.hooks(); // for the hooks
